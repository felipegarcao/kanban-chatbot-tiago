...
export function useConversasDaColuna(
  ...
  return useInfiniteQuery({
    queryKey: ["conversas", sistemaId, status, busca, intervalo.de, intervalo.ate],
    queryFn: ({ pageParam }) => {
      const url = new URL(`/api/sistemas/${sistemaId}/conversas`, window.location.origin);
      url.searchParams.set("status", status);
      url.searchParams.set("de", intervalo.de);
      url.searchParams.set("ate", intervalo.ate);
